...
- 처리: 프로젝트 유형 분류 + 프로젝트명 추출 + 시장 규모 안내
- 전문가 인사이트: 유사 프로젝트 예산/기간/성공 패턴 안내

### Step 2: 타겟 사용자
- 질문: "이 서비스를 주로 누가 사용하게 될까요?"
- 처리: 인구통계 + 기술 수준 추출 + UI/UX 방향 제안
- 전문가 인사이트: 타겟별 UI/UX 전략 (시니어→큰 글씨, MZ→모바일 퍼스트 등)

### Step 3: 핵심 기능
- 질문: "가장 중요한 기능 3가지를 말씀해주세요."
- 처리: 기능 분리 → P1/P2/P3 → 구현 난이도와 예상 기간 안내
- 전문가 인사이트: MVP 전략 + 누락된 필수 기능 제안 (로그인, 결제 등)

### Step 4: 참고 서비스
- 질문: "비슷하게 만들고 싶은 서비스나 앱/웹이 있나요? (없으면 '건너뛰기')"
- 처리: 서비스명 + 참고 포인트 + 차별점 파악

### Step 5: 기술 요구사항
- 질문: "웹으로 만들까요, 앱으로 만들까요, 아니면 둘 다?"
- 처리: 자연어 → 기술 사양 변환 + 최적 기술 스택 추천

### Step 6: 예산과 일정
- 질문: "예산 범위와 완료 시점이 있으신가요? 대략적이어도 괜찮습니다."
- 처리: 예산-기능 간 괴리 분석 + 마일스톤 제안

### Step 7: 추가 요구사항
- 질문: "마지막으로, 개발사에 꼭 전달할 사항이 있나요?"
- 처리: 보안/유지보수/디자인/소스코드 소유권 등 분류
