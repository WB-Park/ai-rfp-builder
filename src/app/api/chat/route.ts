...
async function generateDeepResponse(
  ...
  else if (hasOverview && messageCount >= 3) currentPhase = 'context';  // 최소 3턴 이상일 때 context → specify 전환 가능
...
⚠️ 이 단계에서는 showFeatureSelector=false 유지. 아직 기능을 물을 때가 아닙니다.
이 단계의 목적: 고객의 상황·배경·타겟·경쟁 구도를 충분히 이해하는 것.
최소 2~3개 질문을 이 단계에서 진행한 후 Phase 3으로 넘어갑니다.
...
★★★ 이 단계 시작 시 기능 선택 UI 트리거 (showFeatureSelector=true) ★★★
Phase 2에서 충분한 맥락이 쌓인 후에야 기능 리스트를 보여줍니다.
기능 선택 후:
  ...
  try {
    ...
    // ═══════════════════════════════════════════════════════
    {
      ...
      if (aiResult) {
        ...
        // ═══ Deep 전용: 기능 선택 UI (4중 게이트 — Quick보다 엄격) ═══
        // Deep mode는 충분한 맥락 수집 후에만 기능 리스트 표시
        // 조건: AI가 showFeatureSelector=true 반환 OR phase3 이후 + 최소 3턴 대화
        let selectableFeatures: SelectableFeature[] | null = null;
        const deepUserTurns = messages.filter((m: { role: string }) => m.role === 'user').length;
        const currentDeepPhase = aiResult.deepPhase || deepPhase;
        const phaseAllowsFeatures = ['phase3', 'phase4'].includes(currentDeepPhase);
        const aiRequestsFeatures = aiResult.showFeatureSelector === true;
        const canShowFeatureSelector = !featureSelectorAlreadyShown && !featureJustSubmitted && rfpData.coreFeatures.length === 0
          && (aiRequestsFeatures || (phaseAllowsFeatures && deepUserTurns >= 3));
        ...
        // Deep 전용: 완료 직전 기능 미수집이면 한 번 더 시도 (phase 관계없이 — 완료 시점이므로)
