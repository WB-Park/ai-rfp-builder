...
• 🚫 readyToDefineFeatures=false 무조건 고정 — 절대 true 반환 금지` : ''}
...
• 🚫 readyToDefineFeatures=false 무조건 고정 — 아직 기능 논의 단계가 아님` : ''}
...
• 차별점과 핵심 가치 확인
• readyToDefineFeatures=true 가능 조건: 서비스 비전이 명확하고, 타겟 사용자 방향도 어느 정도 잡혔을 때만. 아직 대화가 두서없으면 false 유지.` : ''}
  ...
  try {
    ...
    // ═══════════════════════════════════════════════════════
    {
      ...
      if (aiResult) {
        ...
        // Deep 전용: 기능 선택 UI
        // ★ 8중 게이트: 하나라도 실패하면 기능 리스트 표시 안 함
        const currentDeepPhase = determineDeepPhase(messages as ChatMessage[], turnCount, rfpData);
        let selectableFeatures: SelectableFeature[] | null = null;
        const shouldShowFeatures = !featureSelectorAlreadyShown  // 1. 이전에 표시한 적 없음
          ...
          && !!rfpData.overview                                    // 7. ★ overview가 반드시 존재해야 함
          && (currentDeepPhase === 'define' || currentDeepPhase === 'refine');  // 8. ★ define/refine 단계에서만
