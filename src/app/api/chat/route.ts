...
function generateSimpleFallback(rfpData: RFPData, userMessage: string): {
  ...
  if (!rfpData.targetUsers) {
    return {
      ...
      quickReplies: ['20~30대 직장인', '전 연령 일반 사용자', '기업 고객 (B2B)'],
      completionReady: true,  // ★ Quick mode: overview + features + targetUsers → 완료
      progressPercent: 80,
    };
  }
  // ★ Quick mode: 3개 핵심 항목 수집 후 나머지는 선택적
  ...
  try {
    ...
    if (aiResult) {
      ...
      if (canShowFeatureSelector) {
        // ★★★ AI showFeatureSelector 의존 제거 ★★★
        // overview가 존재하고 coreFeatures가 비어있으면 자동으로 기능 선택 트리거
        // rfpUpdate로 overview가 방금 이 턴에 설정되었을 수도 있으므로 확인
        ...
        const shouldTriggerFeatures = featureSourceText && featureSourceText.length >= 2;

        if (shouldTriggerFeatures) {
          try {
      ...
      // ★ Quick mode 강제 완료: 5턴 이상이면 무조건 완료, 4턴 이상이고 핵심정보 있으면 완료
