...
기능(${analyzedFeatures.length}개): ${featureList}
...
JSON 응답 (모든 필드는 한국어, 이 프로젝트에 특화된 내용만):
{
  ...
  "constraints": ["이 프로젝트만의 제약 사항. 일반론 절대 금지 (예: '개인정보보호법에 따라 위치정보 수집 시 명시적 동의 필수', '결제 연동 시 PG사 심사 2~3주 소요')"] (정확히 5개)
        ...
        // 병렬 실행 (45초 타임아웃)
        const timeout = setTimeout(() => {}, 45000);
        const [resultA, resultB] = await Promise.allSettled([callA, callB]).finally(() => clearTimeout(timeout));

        // 결과 병합용 객체
        let aiEnhancementA: Record<string, unknown> = {};
        let aiEnhancementB: Record<string, unknown> = {};

        if (resultA.status === 'fulfilled') {
          const contentA = resultA.value.content[0];
          if (contentA.type === 'text') {
            const matchA = contentA.text.match(/\{[\s\S]*\}/);
            if (matchA) try { aiEnhancementA = JSON.parse(matchA[0]); } catch { /* skip */ }
          }
        }
        if (resultB.status === 'fulfilled') {
          const contentB = resultB.value.content[0];
          if (contentB.type === 'text') {
            const matchB = contentB.text.match(/\{[\s\S]*\}/);
            if (matchB) try { aiEnhancementB = JSON.parse(matchB[0]); } catch { /* skip */ }
          }
        }

        const aiEnhancement = { ...aiEnhancementA, ...aiEnhancementB };

        // 가짜 response 객체 생성 (아래 기존 로직과 호환)
        const response = { content: [{ type: 'text' as const, text: JSON.stringify(aiEnhancement) }] };
