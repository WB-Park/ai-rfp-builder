...
[{"name": "기능명", "description": "대화에서 논의된 맥락 반영한 설명", "priority": "P1"}]`
  ...
  // Deep mode는 대화에서 나온 모든 맥락이 PRD 품질을 결정함
...
JSON 형식으로 응답하세요:
{
  ...
  "assumptions": ["★필수★ 이 프로젝트가 성립하기 위한 전제 5개. 대화에서 고객이 언급한 상황/환경을 근거로 작성. 예: '사용자가 스마트폰 사용 가능', 'API 연동 대상 서비스가 안정적으로 운영 중' 등 구체적으로"],
  "constraints": ["★필수★ 대화에서 파악된 이 프로젝트 고유의 제약사항 5개. 고객이 언급한 예산/기간/기술/인력/법규 등의 실제 제약을 반영. 일반론적 제약(예: '성능 최적화 필요') 금지. 예: '초기 개발 예산 3000만원 이내', '3개월 내 MVP 출시 필요', '기존 쇼핑몰 DB와 연동 필수' 등"],
  "techStack": [
...
assumptions: 정확히 5개. ★대화에서 파악된 고객 상황을 근거로 작성. 비어있지 않은 배열 필수★
constraints: 정확히 5개. ★대화에서 고객이 언급하거나 암시한 실제 제약만 작성. 일반론 금지. 비어있지 않은 배열 필수★
